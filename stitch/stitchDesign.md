...
### Talent Cards (Ultra-Compact)
The signature component of this design system.
- **Structure:** 8px padding, 11px border radius.
- **Title:** Rendered in Noto Serif at 0.69rem, bold.
- **Action Button:** Anchored to the top-right corner. Background is always `surface-bright` with a `gold` icon or text.
- **Spacing:** Fixed 5px gap between all internal elements (image, title, stats).

### Sacred Fade Scroll Container
Used for lists and long lore text. Implement using `mask-image` to fade content at the top and bottom edges. Scrollbars must be hidden (`scrollbar-width: none`).

### Buttons
Primary buttons use a Gold background with Surface text. Secondary buttons use a Ghost style with a 1px Essence Purple border and Purple text. Hover states should increase the inner glow intensity.

### Inputs & Fields
Inputs are minimal, featuring only a bottom border in Gold (active) or Surface Bright (inactive). Labels use the `stat-label` typography style, positioned above the field.

### Chips & Badges
Small, rectangular elements with a 2px radius. Backgrounds are low-opacity versions of the Essence Purple or Gold to denote rarity or category without overwhelming the Talent Cards.