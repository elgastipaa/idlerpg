...
export default function FlForgeModeScaffold({
  moduleKey = "polish",
  panel = null,
  ctaLabel = "Aplicar",
  selectedItem = null,
  ...
  const resolvedPanel = panel || {
    material: { label: "Material", value: "-", delta: "-" },
    probability: { label: "Estado", value: "-", capped: false, insufficient: true },
    cost: "-",
  };

  if (!selectedItem) {
    return (
      <div className="fl-crafting-stage fl-crafting-stage--empty fl-forge-mode-module" data-mode={moduleKey}>
        <FlCard
          as="button"
          ...
          aria-label="Seleccionar item para forja"
        >
  ...
  return (
    <div className="fl-forge-mode-module" data-mode={moduleKey}>
      <div className="fl-crafting-stage">
        ...
        <FlCard as="aside" className="fl-crafting-material-panel fl-forge-mode-module__material" variant="compact">
          <div className="fl-crafting-material-section">
            <FlResourceCounter
              className="fl-crafting-material-counter"
              type="material"
              icon="forge"
              label={resolvedPanel.material.label}
              value={resolvedPanel.material.value}
              delta={resolvedPanel.material.delta}
            />
          </div>
          <div className="fl-crafting-material-section">
            <FlResourceCounter
              className="fl-crafting-probability"
              type={resolvedPanel.probability.capped ? "points" : "material"}
              icon="armor"
              label={resolvedPanel.probability.label}
              value={resolvedPanel.probability.value}
              capped={resolvedPanel.probability.capped}
              insufficient={resolvedPanel.probability.insufficient}
            />
          </div>
        </FlCard>
      ...
      <section className="fl-crafting-neutral-section fl-crafting-entropy-panel" aria-label="Entropia de forja">
        <FlEntropyTrack
      ...
      {selectedActionDisabled && selectedActionHint && showPrimaryActions && (
        <FlRequirementHint
